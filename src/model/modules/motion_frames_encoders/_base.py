...
class BaseMotionFramesEncoder(torch.nn.Module, ABC):
    """
    Abstract base class for all motion frames encoder modules.

    The role of an encoder is to process the original motion sequence and
    produce a fixed-size representation for each frame, which will be used
    ...
    def forward(
        ...
        """
        Encodes the motion features into a fixed-size representation.
