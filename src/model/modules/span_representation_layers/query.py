...
class SpanQueryRepresentationLayer(BaseSpanRepresentationLayer):
    ...
    def __init__(self, motion_embed_dim: int, representation_dim: int, max_span_width: int):
        """
        ...
        # NOTE: learnable query vectors for each span width
        self.query_seg = torch.nn.Parameter(torch.randn(motion_embed_dim, max_span_width))
        ...
        for i in range(batch_size):
            for j in range(max_spans):
                if spans_masks[i, j]:
                    ...
                    if 0 < span_width <= self.max_span_width and start >= 0:
                        span_representations[i, j] = all_span_reps[i, start, span_width - 1]
